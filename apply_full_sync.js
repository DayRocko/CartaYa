...
console.log('Patching Avance2135.html with User Instructions...');

// 1. REPLACING handleFileUpload
const hfuStart = 'const handleFileUpload = (e) => {';
const hfuEndMarker = 'reader.readAsArrayBuffer(file);\n  };';
// Fuzzy matching for various possible current versions
...
const newHfu = `const handleFileUpload = (e) => {
  ...
  reader.onload = (evt) => {
    try {
      ...
      // ─── LECTOR UNIVERSAL DE HOJAS (Soporta Título en Fila 1) ───────────────
      ...
      // 3. MODIFICADORES (Grupos y Opciones)
...
// 2. REPLACING useEffect for LOAD
const ueLoadMarker = /React\.useEffect\(\(\) => \{[\s\S]+?localStorage\.getItem\('cartaya_menu_data'\)[\s\S]+?\}, \[\]\);/;
...
if (ueLoadMarker.test(content)) {
    content = content.replace(ueLoadMarker, newUserUe);
    console.log('Load useEffect replaced.');
}

// 3. RESULT BANNER
const bannerPlaceholder = `{(importError || importResult) && (`;
...
// We inject the new banner ABOVE the existing error/result banner as requested
// "Inserta este bloque debajo del botón de carga"
// The original code had the banner block below the buttons.
if (content.indexOf(bannerPlaceholder) !== -1) {
    content = content.replace(bannerPlaceholder, bannerNew);
